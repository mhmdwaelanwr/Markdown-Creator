...
export default function GistHelperDialog({ visible, onClose, initialUrl = '', onSelect, isDark: isDarkProp }) {
  ...
  const handleInsert = () => {
    if (!url.trim()) {
      Alert.alert('Error', 'Please enter a Gist URL');
      return;
    }
  ...
  return (
    <AppDialog
      ...
      subtitle="Paste a Gist URL to embed a code snippet."
      icon="github"
      maxWidth={640}
      scroll={false}
      footer={
    ...
    >
      <View>
        <Text style={[styles.label, { color: isDark ? Colors.textSecondaryDark : Colors.textSecondaryLight }]}>Gist URL</Text>
        <TextInput
          style={inputChrome}
